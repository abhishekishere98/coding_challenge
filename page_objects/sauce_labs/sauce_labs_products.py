...
class all_products_page_locs:
    ...
    item_product_names = '//div[contains(@class,"inventory_item_name")]'
...
class all_products_page_methods(common_methods):
    product_no_cart = 0
    product_nos_cart = []
    product_details = []
    ...
    def validate_products_all_products_page(self):
        ...
        self.test.assertTrue(self.wait_till_element_is_visible(all_products_page_locs.txt_header).is_displayed(),
                             "Swag Labs Header is displayed")
        self.test.assertTrue(self.get_text_from_element(self.find_element("xpath", all_products_page_locs.txt_header))
                             == "Swag Labs", "Swag Labs header is displayed with correct text")
        # Validate Products heading is displayed correctly
        self.test.assertTrue(self.wait_till_element_is_visible(all_products_page_locs.txt_title).is_displayed(),
                             "Products Tile is displayed")
        self.test.assertTrue(self.get_text_from_element(self.find_element("xpath", all_products_page_locs.txt_title))
        ...
        self.test.assertListEqual(item_names, all_products_page_locs.list_items_names,
                                  "All Product Names are displayed")
        ...
        self.test.assertListEqual(item_descs, all_products_page_locs.list_items_desc,
                                  "All Product Descriptions are displayed")
        ...
        self.test.assertTrue(butns_add_to_cart.__len__() == 6, "All Add To Cart buttons displayed")
    ...
    def validate_random_product_added_to_cart(self):
        ...
        self.test.assertTrue(
            self.find_element("XPATH", '(' + all_products_page_locs.item_product + ')[' + str(
                self.product_no_cart) + ']//button').text == "Add to cart",
            "Add Cart button not present before adding product to cart")
        # Click on add to cart
        self.find_element("XPATH", '(' + all_products_page_locs.item_product + ')[' + str(
            self.product_no_cart) + ']//button').click()
        # Check that button is changed to remove
        self.test.assertTrue(
            self.find_element("XPATH", '(' + all_products_page_locs.item_product + ')[' + str(
                self.product_no_cart) + ']//button').text == "Remove",
    ...
    def validate_no_of_products_in_cart(self):
        ...
        :return: None
        """
        self.test.assertTrue(self.product_nos_cart.__len__() == int(
            self.find_element("XPATH", all_products_page_locs.cart_products_no).text),
    ...
    def capture_product_details(self) -> list:
        ...
        for i in self.product_nos_cart:
            product_name = self.find_element("XPATH",
                                             '(' + all_products_page_locs.item_product + ')[' + str(i) + ']//a//div['
                                                                                                         'contains(@class,"inventory_item_name")]').text
            product_desc = self.find_element("XPATH",
                                             '(' + all_products_page_locs.item_product + ')[' + str(i) + ']//div['
